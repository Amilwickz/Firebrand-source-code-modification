...
!Firebrand file
LU_FBRND = GET_FILE_NUMBER() ! by MR
FN_FBRND = TRIM(CHID)//'_fbrnd.csv' ! by MR
...
ELSE
   ...
   OPEN(LU_FBRND,FILE=FN_FBRND,FORM='FORMATTED',STATUS='REPLACE') ! by MR   
   ...
   IF (N_ZONE_TMP>0) THEN
      ...
      WRITE(LU_FBRND,TCFORM) 'TIME','TIME_STEP','MASS_LOSS_RATE', 'DRY_MASS_LOSS','FBRAND_PER_KG_DRY_MASS','TOTAL_NO_FBRAND'! by MR
   ELSE
      WRITE(LU_HRR,TCFORM) 'Time','HRR','Q_RADI','Q_CONV','Q_COND','Q_DIFF','Q_PRES','Q_PART','Q_ENTH','Q_TOTAL',&
...
REAL(FB) :: TIME_STEP_SIZE ! by MR
REAL(FB) :: DRY_ML ! by MR
...
ELSE
   ...
   TIME_STEP_SIZE=STIME-PREV_TIME ! by MR  
   PREV_TIME=STIME ! by MR   
   DRY_ML=M_DOT_TOTAL(2)*TIME_STEP_SIZE ! by MR                   
   N_FBRAND_GEN_TOT_ML=floor(DRY_ML*N_FBRAND_GEN_PKG_ML) ! by MR                        
   WRITE(LU_FBRND,TCFORM) STIME,TIME_STEP_SIZE,M_DOT_TOTAL(2),DRY_ML*1.0_EB,N_FBRAND_GEN_PKG_ML*1.0_EB, N_FBRAND_GEN_TOT_ML*1.0_EB ! by MR
ENDIF
